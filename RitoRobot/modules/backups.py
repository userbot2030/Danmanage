...
# from RitoRobot.modules.sql import warns_sql as warnssql
import RitoRobot.modules.sql.blacklist_sql as blacklistsql

# from RitoRobot.modules.sql import cust_filters_sql as filtersql
# import RitoRobot.modules.sql.welcome_sql as welcsql
import RitoRobot.modules.sql.locks_sql as locksql
import RitoRobot.modules.sql.notes_sql as sql

# from RitoRobot.modules.rules import get_rules
import RitoRobot.modules.sql.rules_sql as rulessql
from RitoRobot import EVENT_LOGS, LOGGER, OWNER_ID, SUPPORT_CHAT, dispatcher
from RitoRobot.__main__ import DATA_IMPORT
from RitoRobot.modules.connection import connected
from RitoRobot.modules.helper_funcs.alternate import typing_action
from RitoRobot.modules.helper_funcs.chat_status import user_admin
from RitoRobot.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("KynanRobot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("KynanRobot{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `RitoRobot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("RitoRobot{}.backup".format(chat_id))  # Cleaning file
...
*Only for group owner:*
 ...
 ᐉ /import: Reply to the backup file for the butler / emilia group to import as much as possible, making transfers very easy! \
 Note that files / photos cannot be imported due to telegram restrictions.

 ᐉ /export: Export group data, which will be exported are: rules, notes (documents, images, music, video, audio, voice, text, text buttons) \
