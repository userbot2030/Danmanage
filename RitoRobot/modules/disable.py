...
from RitoRobot.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from RitoRobot.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from RitoRobot.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "RitoRobot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "RitoRobot.modules." + args[0].rsplit(".", 1)[0]
    ...
    __help__ = """
    ᐉ /cmds*:* check the current status of disabled commands

    *Admins only:*
    ᐉ /enable <cmd name>*:* enable that command
    ᐉ /disable <cmd name>*:* disable that command
    ᐉ /enablemodule <module name>*:* enable all commands in that module
    ᐉ /disablemodule <module name>*:* disable all commands in that module
    ᐉ /listcmds*:* list all possible toggleable commands
    """
