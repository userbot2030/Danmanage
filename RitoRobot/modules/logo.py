...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./RitoRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await telethn.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [͔ ꝛɪᴛᴏ ꝛᴏʙᴏᴛ](https://t.me/Rito_Robot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./RitoRobot/resources/fonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if event.reply_to_msg_id:
        ...
        pics = []
        async for i in telethn.iter_messages(
            "@AllLogoRitoRobot", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
